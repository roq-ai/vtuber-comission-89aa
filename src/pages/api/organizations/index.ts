...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.content_creator?.length > 0) {
      const create_content_creator = body.content_creator;
      body.content_creator = {
        create: create_content_creator,
      };
    } else {
      delete body.content_creator;
    }
